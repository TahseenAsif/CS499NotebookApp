...
//will use signout not necessary right now
...
//creates notification to notify user
function notifyUser(msg_title, msg_body){
    new Notification({
        title: msg_title,
        body: msg_body
    }).show();
...
async function generateDataJSON(){
    ...
    //console.log(userID);
    if(!userID){
        var toJSON = JSON.stringify(saveData, null, 2);
    ...
    */
    ...
    notifyUser("Saved", "Your file(s) has been successfully saved!");
}
...
ipcMain.on("sign-in", (event, email, password) => {
    signIn(email, password, (uid) => {
        ...
        updateWindowApp();
        setTimeout(() => {
            mainWindow.webContents.send("loadUserData", docSnap.data());
        }, 400);
    });
})
...
ipcMain.on("guest", () => {
    ...
    setTimeout(() => {
        mainWindow.webContents.send("loadUserData", "guest");
    }, 1000);
})
...
ipcMain.on("runPaint", () => {
    //check if paint window already exist
    if(paintWindow){
        //show it to user
        paintWindow.moveTop();
    }
...
//ipcMain listening for tab creation for text and code editors
ipcMain.on("createTextTab", () => {
    mainWindow.webContents.send("newTextTab", "Text tab created!");
});

ipcMain.on("loadTextTab", async (event, data) => {
    mainWindow.webContents.send("loadTextData", data);
});

ipcMain.on("saveTextTabRequest", async (event, index) => {
    mainWindow.webContents.send("saveTextData", index); 
});

ipcMain.on("saveTextTab", async (event, data, last_request) => {
    console.log("debug here!");
    saveData.text.push(data);
    //check if it is the last request
    if(last_request){
        //generate a new data.json file if it is just saving a single tab
        generateDataJSON();
    }
});

ipcMain.on("createCodeTab", () => {
    mainWindow.webContents.send("newCodeTab", "Code tab created!");
});
...
log_file.close()
    ...
    notifyUser("Saved", "Your file has been successfully saved!");
});
